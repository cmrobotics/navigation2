...
BtNavigator::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_activate() || !pose_navigator_->on_activate() // || !nodes_navigator_->on_activate()
      ) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_deactivate() || !pose_navigator_->on_deactivate() // || !nodes_navigator_->on_deactivate()
      ) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_cleanup() || !pose_navigator_->on_cleanup() // || !nodes_navigator_->on_cleanup()
      ) {
    return nav2_util::CallbackReturn::FAILURE;
  }
